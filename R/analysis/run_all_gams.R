...
library(ggcorrplot)
corr <- round(cor(dt_cor), 1)

ggcorrplot(corr, hc.order = TRUE, type = "lower",
           lab = TRUE)

ggplot() +
  geom_point(data = dt_mod, aes(y = distance_enclosure, x = min_dist_enclosure))

ggplot() +
  geom_point(data = dt_mod, aes(y = min_dist_enclosure, x = min_dist_enclosure))
...
names(dt_mod %>% dplyr::select(contains("scaled")))
...
combinations <- generate.combinations(vars, max.n = 4)
...
for(i in 1:nrow(dt.tier)){
  ...
  dt.sub <- dt_mod %>% filter(eval(parse(text = subset)))
...
for(i in 1:length(pair.combs)){
  ...
  var1 <- dt_mod %>% dplyr::select(all_of(dt.tmp$var1)) %>% pull()
  var2 <- dt_mod %>% dplyr::select(all_of(dt.tmp$var2)) %>% pull()
...
for(model_group in unique(guide$model_group)){
  ...
  var_names <- bm.spec %>% dplyr::select(vars) %>% 
  ...
  var_names <-  gsub(",bs='re'", "", var_names)
  var_names <-  gsub(", bs='re'", "", var_names)
  var_names <-  gsub("\\) ", "", var_names)
  var_names <-  gsub("\\)", "", var_names)
  var_names <-  gsub("enclosure_name,", "", var_names)
  var_names <-  gsub("bs='re'", "", var_names)
  var_names <-  gsub(" ", "", var_names)
  var_names <-  gsub("1", "", var_names)
  var_names <- gsub(", k = 3", "", var_names)
  var_names <- gsub(",k=3", "", var_names)
  
  
  
  
  var_names <- var_names %>% as.data.frame() %>% filter(!grepl("'re'", .)) %>% pull()
  
  var_names <- unique(var_names)
  if(length(var_names) == 1){if(var_names == ""){var_names <- "intercept"}}
