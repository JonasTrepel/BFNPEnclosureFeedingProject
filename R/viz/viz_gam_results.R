...
#Vegetation 
veg.ids <- unique(bm.spec[sphere == "Vegetation" & grepl("min_dist_enclosure", bm.spec$vars), ]$formula_id)
dt.pred.veg <- pred[formula_id %in% c(veg.ids) & clean_var == "min_dist_enclosure", ]  %>% 
  ...
  significance = case_when(
    ...
    response == "c" ~ "not included", 
    response == "p" ~ "significant", 
    response == "k" ~ "significant", 
    response == "mg" ~ "significant", 
    response == "ca" ~ "not included", 
    response == "na" ~ "non significant",
    response == "cn" ~ "significant",
    response == "np" ~ "not included"
  ),
  ...
  filter(significance != "not included") %>% 
  mutate(nutrient = factor(nutrient, levels = c(
    "N", "C", "P", "K", "Mg", "Ca", "Na", "C:N", "N:P")))
...
#Soil 
soil.ids <- unique(bm.spec[sphere == "Soil"  & grepl("min_dist_enclosure", bm.spec$vars), ]$formula_id)
dt.pred.soil <- pred[formula_id %in% c(soil.ids) & clean_var == "min_dist_enclosure", ]  %>% 
  mutate(nutrient = str_to_sentence(response), 
         significance = case_when(
           response == "n" ~ "significant", 
           response == "c" ~ "significant", 
           response == "p" ~ "not included", 
           response == "k" ~ "not included", 
...
#Vegetation 
veg.ids <- unique(bm.spec[sphere == "Vegetation"  & grepl("deadWoodChangeYear_scaled", bm.spec$vars), ]$formula_id)
...
#Soil 
soil.ids <- unique(bm.spec[sphere == "Soil" & grepl("deadWoodChangeYear_scaled", bm.spec$vars), ]$formula_id)
